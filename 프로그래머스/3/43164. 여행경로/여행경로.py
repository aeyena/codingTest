def solution(tickets):
    ...
    def getPath(tickets, path):
        ...
        # len(tickets) == 0이 아니었으므로 티켓이 남아있다는건데 나아갈 공항이 없다는 것은 유효한 루트가 아니라는 뜻이므로 실패의 의미에서 빈 리스트 리턴
        if idx == -1:
            return []
        ...
        while tickets[idx][0] == now:
            next_path = getPath(tickets[:idx] + tickets[idx+1:], path + [tickets[idx][1]])
            if next_path != []:
                return next_path
